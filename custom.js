...
// add a record to the myJournalArray array defined above
...
function addRecord(events, werewolf) {
  ...
  myJournalArray.push({events, werewolf});
}
...
// taking frequency of transformation(variable 1) with respect to the event of eating pizza(variable 2) and using this frequency as the input of our phi coefficient. NB. using the myJournalArray array in myJournalArray.js as the test date
...
// looping over all the records and tally how many times the pizza was eaten in relation to warewolf transformations. This will enable us to extract a two-by-two table for a specific record from the myJournalArray

function tableFor(event, myJournalArray) {
  let table = [0, 0, 0, 0];
  for (let i = 0; i < myJournalArray.length; i++) {
    let record = myJournalArray[i], index = 0;
...
// console.log(tableFor("pizza", myJournalArray));
// → [11, 0, 0, 3]
...
/*myJournalArrayEvents*/ 


// NB: where the below syntax of the for loop is used, with the word "of" after a variable definition,it will loop over the elements of the value given after "of", in this case "myJournalArray" is stated after "of" which is an array, therefore there will be a loop over the elements of "myJournalArray"

// for (let record of myJournalArray) {
...
function myJournalArrayEvents(myJournalArray) {
  let events = [];
  for (let record of myJournalArray) {
